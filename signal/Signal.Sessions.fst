...
= assert (includes_ids [P p] [V p si vi]);
  A.concat v.send_chain_key (A.concat (A.literal_to_labeled_bytes #signal_global_usage #i #(readers [V p si vi]) (Nat v.send_counter)) 
...
let parse_send_session_data st : result (option send_state) = 
  ...
  (match bytes_to_literal sc with
  | Success (Nat sct) -> Success (Some ({send_chain_key = sck; send_counter = sct; our_ephemeral_private_key = oek}))
  | _ -> Success (None))
...
let serialize_recv_session_data (i:nat) (p:principal) (si:nat) (vi:nat) (peers:session_peers) (root_key:bytes)
    ...
    : bytes 
= assert (includes_ids [P p] [V p si vi]);
  A.concat v.recv_chain_key (A.concat (A.literal_to_labeled_bytes #signal_global_usage #i #(readers [V p si vi]) (Nat v.recv_counter))
...
let parse_recv_session_data st : result (option recv_state) = 
  ...
  (match bytes_to_literal rc with
  | Success (Nat rct) -> Success (Some ({recv_chain_key = rck; recv_counter = rct; their_ephemeral_public_key = tek}))
  | _ -> Success None)
...
let parse_session_st serialized_session =
  ...
  | String "SendSession" ->
    ...
    (match bytes_to_literal a, bytes_to_role rl with
    | Success (String peer), Success role ->
...
let serialize_valid_session_st i p si vi st =
  ...
  | Session peers root_key (Some send) None ->
    ...
    A.concat (A.literal_to_labeled_bytes #signal_global_usage #i #(readers [V p si vi]) (String "SendSession")) 
      (A.concat (A.literal_to_labeled_bytes #signal_global_usage #i #(readers [V p si vi]) (String peers.peer)) 
  ...
  | Session peers root_key None (Some recv) ->
    ...
    A.concat (A.literal_to_labeled_bytes #signal_global_usage #i #(readers [V p si vi]) (String "RecvSession")) 
      (A.concat (A.literal_to_labeled_bytes #signal_global_usage #i #(readers [V p si vi]) (String peers.peer)) 
    	(A.concat (role_to_labeled_bytes #i #(readers [V p si vi]) peers.role) 
    	  (A.concat peers.our_identity_public_key 
    	    (A.concat peers.their_identity_public_key 
