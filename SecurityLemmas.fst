...
val corrupt_principals_have_publishable_state_at: pr:preds -> t:trace -> trace_index:nat ->
...
let corrupt_principals_have_publishable_state_at pr t i = 
  let corrupt_principal_has_publishable_state (pr:preds) (i:nat) (j:nat) (p:principal) (v:version_vec) (s:state_vec) (session_index:nat) (b:bytes) :
...
val publishable_before_is_publishable : pr:preds -> h:trace -> i:nat ->
...
val corrupted_before_is_publishable : pr:preds -> h:trace -> i:nat -> 
    Lemma (requires (valid_trace pr h /\ later_than (trace_len h) i))
...
val attacker_only_derives_public_values_:
    pr:preds -> h:trace -> i:nat -> steps:nat -> Lemma
...
let rec attacker_only_derives_public_values_ pr h i steps = 
  if steps = 0 then ( 
        literals_are_publishable_forall pr.global_usage; 
...
val attacker_only_derives_public_values:
    pr:preds -> h:trace -> i:nat -> steps:nat -> b:bytes -> Lemma
...
let attacker_preserves_validity pr t0 t1 = 
  ...
  assert (Att.attacker_modifies_trace t0 t1 /\ valid_trace pr t0 ==>
	 (forall (i:nat) (t:bytes) (s:principal) (r:principal). i < trace_len t1 ==> (was_message_sent_at i s r t ==> (is_publishable pr.global_usage i t))));
  assert (Att.attacker_modifies_trace t0 t1 /\ valid_trace pr t0 ==>
	 (forall (i:nat) (p:principal) (v:version_vec) (s:state_vec). i < trace_len t1 ==> (state_was_set_at i p v s ==> (state_inv pr i p v s))));
  assert (Att.attacker_modifies_trace t0 t1 /\ valid_trace pr t0 ==>
	 (forall (i:nat) (s:principal) (e:event). i < trace_len t1 ==> (did_event_occur_at i s e ==> (event_pred_at pr i s e))))
