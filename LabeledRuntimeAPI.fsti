...
noeq type trace_preds (g:global_usage) = {
  can_trigger_event: nat -> principal -> event -> Type0;
  session_st_inv: nat -> principal -> nat -> nat -> bytes -> Type0;
  session_st_inv_lemma: i:nat -> p:principal -> si:nat -> vi:nat -> st:bytes ->
		  Lemma (session_st_inv i p si vi st ==>
			 is_msg g i st (readers [V p si vi]));
  session_st_inv_later: i:nat -> j:nat -> p:principal -> si:nat -> vi:nat -> st:bytes ->
...
let state_inv (pr:preds) (idx:nat) (prin:principal) (version_vec:version_vec) (st:state_vec) : Type0 =
...
let session_st_inv_later (pr:preds) (i:nat) (j:nat) (p:principal) (si:nat) (vi:nat) (st:bytes) :
...
let state_inv_later (pr:preds) (i:nat) (j:nat) (p:principal) (vv:version_vec) (st:state_vec) :
...
val valid_trace_respects_state_inv: (pr:preds) -> (trace:trace) -> (i:nat) -> (p:principal) -> (v:version_vec)-> (s:state_vec) ->
...
val rand_gen: #pr:preds -> l:label -> u:usage ->
  LCrypto (i:nat & secret pr.global_usage i l u) pr
...
val send: #pr:preds -> #i:nat -> sender:principal -> receiver:principal ->
	  message:msg pr.global_usage i public -> LCrypto nat pr
...
val receive_i: #pr:preds -> index_of_send_event:nat -> receiver:principal ->
	  LCrypto (now:nat & sender:principal & msg pr.global_usage now public) pr
    (requires (fun t0 -> True))
...
val new_session:
  #pr:preds -> #i:nat -> p:principal -> si:nat -> vi:nat -> st:bytes ->
  LCrypto unit pr
  (requires fun t0 -> trace_len t0 == i /\ pr.trace_preds.session_st_inv i p si vi st)
  (ensures fun t0 r t1 -> trace_len t1 == trace_len t0 + 1)

val update_session:
  #pr:preds -> #i:nat -> p:principal -> si:nat -> vi:nat ->
  st:bytes ->
  LCrypto unit pr
  (requires fun t0 -> trace_len t0 == i /\ pr.trace_preds.session_st_inv i p si vi st)
  (ensures fun t0 r t1 -> trace_len t1 == trace_len t0 + 1)
...
val get_session:
  #pr:preds -> #i:nat -> p:principal -> si:nat ->
...
val find_session:
  #pr:preds -> #i:nat -> p:principal -> f:(si:nat -> vi:nat -> st:bytes -> bool) ->
